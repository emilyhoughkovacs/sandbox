...
import random
"""
make_phrase: makes n-word phrase from 2 separate texts.
each word in n-word phrase must be more than 5 letters long and occur in both texts.

define what will be your phrase1 and phrase2 above.
function takes number of desired words in phrase as third parameter, n
"""
def make_phrase(phrase1, phrase2, n):
    set1 = set()
    set2 = set()
    both = set()
    # r"[\w']+" is regex to match newline and space
    phrase1 = re.findall(r"[\w']+", mmg)
    phrase2 = re.findall(r"[\w']+", geb)
    for word in phrase1:
        if len(word) >= 5:
            set1.add(word)
    for word in phrase2:
    ...
    if len(both) >= n:
        print ' '.join(random.sample(both, n))
    else:
        print "Not enough common words"